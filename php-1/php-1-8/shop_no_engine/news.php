...
//Получим все новости
$result=mysqli_query($connect,"SELECT idx,news_title,prev FROM `news` WHERE 1;");
...
<div id="container">
	...
	<? include "auth_form.php";?>
		...
		echo
				...
				"
                    <h3 class='item-name'>{$row[news_title]}</h3>
                    <p>{$row[prev]}</p>
					<a href='show_news.php?idx={$row[idx]}'>Подробнее</a>
					<br>
